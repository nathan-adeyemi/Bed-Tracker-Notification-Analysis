repair_prob_dp  <-
  function(data_seq,
    ...
    for (i in seq_along(dp_struc)) {
      ...
      for (i in 2:length(dp_struc)) {
        ...
        for (j in seq(length(dp_struc[[i]]))) {
          ...
          if (c_i > sum(theta[seq(i - 1)]) &
              i > 2 & heuristic_cutoff) {
            dp_struc[[i]][[j]] <- dp_struc[[i]][[j - 1]]
          } else {
            prev_repairs  <-
            ...
            for (prev_repair in seq_along(prev_repairs)) {
              ...
              if (c_prev > 0 & c_prev <= delta) {
                ...
                prev_repair_likes <-
                  sapply(
                    # Likelihood of trajectory up to point i given maximum likelihood repair up to i-1
                ...
                prev_repair_likes <-
                  sapply(
                    ...
                    FUN = function(repair) {
                      ...
                      poss_future_repairs  <-
                        if (!is.na(data_seq[i + 1])) {
                          seq(max(0, data_seq[i + 1] - min(delta - c_i, theta[i + 1])),
                              min(K, data_seq[i + 1] + min(delta - c_i, theta[i + 1])))
                      ...
                      if (is.infinite(prev_repair_likes[repair] + p_max)) {
                        return(-Inf)
                      } else {
    ...
    for (index in seq(length(data_seq), 1)) {
      ...
      repaired_state <-
        if (index == length(data_seq)) {
          if(is.list(dp_struc[[i]][[delta]])){
            state_struc <- dp_struc[[i]][[delta]][[1]]
          } else {
            state_struc <- dp_struc[[i]][[delta]]
          }
          as.numeric(names(which.max(state_struc[which.max(state_struc)])))
        } else{
    ...
    result_list <- list(
      ...
      budget = curr_delta,
      runtime = as.numeric(toc(quiet = T)$toc - start),
